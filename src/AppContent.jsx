...
const AppContent = ({ user }) => {
  ...
  const query = tasksRef.where("userId", "==", user.uid);
  const [tasksSnapshot, loading, error] = useCollection(query);
  ...
  const addTask = (task) => {
    tasksRef.add({ ...task, userId: user.uid });
    setShowForm(false);
  };
  ...
  const updateTask = (updatedTask) => {
    tasksRef.doc(updatedTask.id).update(updatedTask);
    setEditTask(null);
    setShowForm(false);
  };
  ...
  const toggleTaskCompletion = (id) => {
    ...
    if (task) {
      tasksRef.doc(id).update({ completed: !task.completed });
    }
  };
